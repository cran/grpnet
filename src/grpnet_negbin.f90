!   grpnet_negbin.f90 - group elastic net (negative binomial)
...
!   theta = additional parameter:  Var = mu + mu^2/theta


SUBROUTINE grpnet_negbin(nobs, nvars, x, y, w, off, ngrps, gsize, pw, alpha, &
...
! --------------- GENERATE LAMBDA --------------- !
    IF (maxlam <= macheps) THEN
        ...
        ! calculate deviance !
        CALL grpnet_negbin_dev(nobs, y, mu, w**2, theta, devs(i))
        ! calculate deviance !
...
SUBROUTINE grpnet_negbin_dev(nobs, y, mu, wt, theta, dev)
    IMPLICIT NONE
    INTEGER nobs, i
    DOUBLE PRECISION y(nobs), mu(nobs), wt(nobs), theta, dev
    dev = 0.0D0
    DO i=1,nobs
        IF (y(i) > 0) THEN
