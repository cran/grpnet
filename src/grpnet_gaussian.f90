!   grpnet_gaussian.f90 - group elastic net (gaussian)
...
SUBROUTINE grpnet_gaussian(nobs, nvars, x, y, w, off, ngrps, gsize, pw, alpha, &
...
! --------------- POST PROCESSING --------------- !
    ...
    IF (intercept == 1) THEN
        ...
        nulldev = SUM((y - w * SUM(y * w) / nobs)**2)
    ELSE
        nulldev = SUM((y - off)**2)
    END IF
! --------------- POST PROCESSING --------------- !
