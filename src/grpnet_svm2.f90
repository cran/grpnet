!   grpnet_svm2.f90 - group elastic net (squared support vector machine)
...
SUBROUTINE grpnet_svm2(nobs, nvars, x, y, w, off, ngrps, gsize, pw, alpha, &
...
! --------------- LOCAL DEFINITIONS --------------- !
    ...
    DOUBLE PRECISION xev(ngrps), znorm, bnorm, yo(nobs), mu(nobs), temp(nobs,2)
...
! --------------- GENERATE LAMBDA --------------- !
    IF (maxlam <= macheps) THEN
        ...
        IF (nzgrps(1) > 0) THEN
            DO WHILE(iter < maxit)
                ...
                DO k=1,ngrps
                    ...
                    mu = mu + MATMUL(x(:,ia(k):ib(k)), difbeta(ia(k):ib(k)))
                    temp(:,2) = 1.0D0 - yo * mu
                    r = 2.0D0 * y * MAXVAL(temp, DIM = 2)
        ...
        ! calculate deviance !
        CALL grpnet_svm2_dev(nobs, y, mu, w**2, devs(1))
        ! calculate deviance !
...
! --------------- ITERATIVE WORK --------------- !
    DO i=1,nlam
        ...
        ! calculate deviance !
        CALL grpnet_svm2_dev(nobs, y, mu, w**2, devs(i))
        ! calculate deviance !
...
! --------------- POST PROCESSING --------------- !
    ...
    IF (intercept == 1) THEN
        ...
        mu = w * SUM(y * w) / nobs
        CALL grpnet_svm2_dev(nobs, y, mu, w**2, nulldev)
    ELSE
        mu = off
        CALL grpnet_svm2_dev(nobs, y, mu, w**2, nulldev)
    END IF
...
SUBROUTINE grpnet_svm2_dev(nobs, y, mu, wt, dev)
    IMPLICIT NONE
    INTEGER nobs
    DOUBLE PRECISION y(nobs), mu(nobs), wt(nobs), dev, temp(nobs,2)
    temp = 0.0D0
    temp(:,2) = 1.0D0 - mu * y / wt
    dev = 2.0D0 * SUM(wt * MAXVAL(temp, DIM = 2)**2)
END SUBROUTINE
