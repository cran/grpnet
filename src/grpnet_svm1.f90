!   grpnet_svm1.f90 - group elastic net (quadratically smoothed support vector machine)
...
SUBROUTINE grpnet_svm1(nobs, nvars, x, y, w, off, ngrps, gsize, pw, alpha, &
...
! --------------- GENERATE LAMBDA --------------- !
    IF (maxlam <= macheps) THEN
        ...
        ! calculate deviance !
        CALL grpnet_svm1_dev(nobs, y, r, w**2, theta, devs(1))
        ! calculate deviance !
...
! --------------- ITERATIVE WORK --------------- !
    DO i=1,nlam
        ...
        ! calculate deviance !
        CALL grpnet_svm1_dev(nobs, y, r, w**2, theta, devs(i))
        ! calculate deviance !
...
! --------------- POST PROCESSING --------------- !
    ...
    IF (intercept == 1) THEN
        ...
        ELSE
            r = y - w * SUM(y * w) / nobs
            CALL grpnet_svm1_dev(nobs, y, r, w**2, theta, nulldev)
        END IF
    ELSE
        r = y - off
        CALL grpnet_svm1_dev(nobs, y, r, w**2, theta, nulldev)
...
SUBROUTINE grpnet_svm1_dev(nobs, y, mu, wt, theta, dev)
    IMPLICIT NONE
    INTEGER nobs, i
