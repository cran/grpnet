R_grpnet_poisson <-
  function(nobs, nvars, x, y, w, off, ngrps, gsize, pw, alpha, 
    ...
    # grpnet_poisson.f90 translation to R
    ...
    if(maxlam <= macheps){
      ...
      # ! calculate deviance ! #
      devs[i] <- R_grpnet_poisson_dev(nobs, y, mu, w^2)
      # ! calculate deviance ! #
    ...
    nulldev <- R_grpnet_poisson_dev(nobs, y, mu, w^2)
...
R_grpnet_poisson_dev <-
  function(nobs, y, mu, wt, dev){
    dev <- 0.0
    for(i in 1:nobs){
      if(y[i] > 0) {
        dev <- dev + 2.0 * wt[i] * (y[i] * log(y[i] / mu[i]) - (y[i] - mu[i]))
      } else {
        dev <- dev + 2.0 * wt[i] * mu[i]
      }
    }
    return(dev)
  } # R_grpnet_poisson_dev.R