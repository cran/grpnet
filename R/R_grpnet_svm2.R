R_grpnet_svm2 <-
  function(nobs, nvars, x, y, w, off, ngrps, gsize, pw, alpha, 
    ...
    # grpnet_svm2.f90 translation to R
    ...
    if(maxlam <= macheps){
      ...
      if(nzgrps[i] > 0L){
        ...
        while(iter < maxit){
          ...
          for(k in 1:ngrps){
            ...
            mu <- mu + x[,ia[k]:ib[k]] %*% difbeta[ia[k]:ib[k]]
            r <- 2 * y * pmax(0, 1 - yo * mu)
      ...
      # ! calculate deviance ! #
      devs[i] <- R_grpnet_svm2_dev(nobs, y, mu, w^2)
      # ! calculate deviance ! #
    ...
    if(intercept == 1L){
      ...
      mu <- w * sum(y * w) / nobs
      nulldev <- R_grpnet_svm2_dev(nobs, y, mu, w^2)
    } else {
      nulldev <- R_grpnet_svm2_dev(nobs, y, off, w^2)
    }
...
R_grpnet_svm2_dev <-
  function(nobs, y, mu, wt, dev = 0.0){
    2.0 * sum(wt * pmax(0, 1 - mu * y / wt)^2)
  } # R_grpnet_svm_dev.R