  ...
  function(nobs, nvars, x, y, w, off, ngrps, gsize, pw, alpha, 
    ...
    if(maxlam <= macheps){
      ...
      if(nzgrps[i] > 0L){
        ...
        iter <- 1L
        if(intercept == 1L){
          ibeta[i] <- log( sum(y * w^2) / sum((1 - y) * w^2) )
          eta <- eta + ibeta[i]
          mu <- 1 / (1 + exp(-eta))
          r <- w * (y - mu)
        }
      ...
      # ! calculate deviance ! #
      devs[i] <- R_grpnet_binomial_dev(nobs, y, mu, w^2)
      # ! calculate deviance ! #
    ...
    nulldev <- R_grpnet_binomial_dev(nobs, y, mu, w^2)
...
R_grpnet_binomial_dev <-
  function(nobs, y, mu, wt, dev){
    ...
    for(i in 1:nobs){
      ...
      dev <- dev + 2 * wt[i] * (val0 + val1)
    }
    return(dev)
  } # R_grpnet_binomial_dev.R